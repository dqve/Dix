			...
			if (meter.listening && meter.handle.sample) {
				...
				if (deb >= 30) {
					...
					theme: 'warning'
						...
						title: ""+ deb + " dB",
						message: "Dix says you've passed your noise threshhold @ " + deb + " dB",
					});
				}
				
				
				if ( deb >= 30) {
					document.getElementById("display").innerHTML = deb + "dB";
				}
				...
				if (bells.length == 1000) {
					bells.sort((el1, el2) => el1 - el2);
					var deci = bells[1000];
					bells.length = 0;
				}
				
				dispatch(meter, 'sample', [dB, percent, value, deb, deci]);
			}
